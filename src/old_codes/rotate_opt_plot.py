...
def initialize_W_R(N, p_high, p_low, S, WR_tuned=False, desired_radius=0.9):
    """
    Build a recurrent matrix with *symmetric* blocks and scale it.
    shape–shape and color–color blocks share the same random distribution.
    shape->color and color->shape also share the same random distribution.
    """
    halfN = N // 2
    W_R = np.zeros((N, N))
    ...
    # Copy shape–shape block onto color–color block for perfect symmetry
    ...
    # Copy shape->color onto color->shape block
