...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------
    # Assume you've already run your main code and have:
    #   1) pca_grid (the PCA object)
    #   2) responses_grid_unmod
    #   3) proj_unmod = pca_grid.transform(responses_grid_unmod)  # shape [121, 3]
    #   4) shape_vals, color_vals (each length 11)
    #   5) color_list = array of length 121 with color values
    #  
    # We'll also construct shape_list similarly for plotting by shape.
    # ------------------------------------------------------

    # Build shape_list to match the shape of proj_unmod
    shape_list = []
    for s in shape_vals:
        for c in color_vals:
            shape_list.append(s)
    shape_list = np.array(shape_list)  # shape [121,]

    # =====================================================
    # 1) 3D PCA visualization (PC1, PC2, PC3), colored by color_list
    # =====================================================
    ...
    ax.scatter(
        ...
        proj_unmod[:, 2],   # PC3
        c=color_list,
        cmap='viridis',      # or any preferred colormap
        s=20
    )
    ...
    ax.set_ylabel("PC2")
    ax.set_zlabel("PC3")
    ax.set_title("3D PCA Space (Unmodulated)")

    # Save high-res
    plt.savefig("pca_3d_unmodulated.png", dpi=300, bbox_inches="tight")
    plt.show()

    # =====================================================
    # 2) 2D PCA plot (PC1 vs PC2), colored by COLOR (blue colormap)
    # =====================================================
    fig2 = plt.figure(figsize=(6, 5))
    plt.scatter(
        proj_unmod[:, 0],   # PC1
    ...
    plt.xlabel("PC1")
    plt.ylabel("PC2")
    plt.title("2D PCA (Unmodulated) — Colored by Color")
    # No colorbar needed
    plt.grid(True)

    plt.savefig("pca_2d_color_unmodulated.png", dpi=300, bbox_inches="tight")
    plt.show()

    # =====================================================
    # 3) 2D PCA plot (PC1 vs PC2), colored by SHAPE (red colormap)
    # =====================================================
    ...
    plt.xlabel("PC1")
    plt.ylabel("PC2")
    plt.title("2D PCA (Unmodulated) — Colored by Shape")
    # No colorbar needed
    plt.grid(True)

    plt.savefig("pca_2d_shape_unmodulated.png", dpi=300, bbox_inches="tight")
    plt.show()
