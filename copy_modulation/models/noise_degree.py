...
# -------------------------------------------------
# 3) Original Modulated (for reference)
# -------------------------------------------------
def compute_modulated_responses(W_R, W_F, S, stimuli_grid):
    sel_diff = S[:, 1] - S[:, 0]
    g_vector = 1 + 0.2 * sel_diff
    G = np.diag(g_vector)
    ...
    for (sh, co) in stimuli_grid:
        F = np.array([sh, co])
        out.append(inv_mat @ (G_WF @ F))
    return np.array(out)

# -------------------------------------------------
# 4) New PC1-based Modulation Function
# -------------------------------------------------
def compute_modulated_responses_pc1_new(W_R, W_F, S, stimuli_grid, pc1, alpha=1.0):
    """
    Define:
      diff_ratio[i] = (max(S[i,1],0) - max(S[i,0],0)) / (max(S[i,1],0) + max(S[i,0],0) + eps)
      overlap[i] = |pc1_unit[i]|, normalized.
      
    Then gain:
      g[i] = 1 + alpha * diff_ratio[i] * overlap[i]
      
    This boosts neurons with high color preference and pushes those with high shape preference.
    """
    eps = 1e-9
    N = W_R.shape[0]
    I = np.eye(N)
    pc1_unit = pc1 / (np.linalg.norm(pc1) + eps)
    color_part = np.maximum(S[:, 1], 0)
    shape_part = np.maximum(S[:, 0], 0)
    diff_ratio = (color_part - shape_part) / (color_part + shape_part + eps)
    overlap = np.abs(pc1_unit)
    overlap /= (np.max(overlap) + eps)
    
    g_vector = 1 + alpha * diff_ratio * overlap
    g_vector = np.clip(g_vector, 0.1, 10.0)
    
    G = np.diag(g_vector)
    I_minus = I - G @ W_R
    if np.linalg.cond(I_minus) > 1 / np.finfo(I_minus.dtype).eps:
        raise ValueError("(I - G W_R) nearly singular; adjust alpha.")
    inv_I_minus = np.linalg.inv(I_minus)
    G_WF = G @ W_F
    mod_resp = []
    for (sh, co) in stimuli_grid:
        F = np.array([sh, co])
        mod_resp.append(inv_I_minus @ (G_WF @ F))
    return np.array(mod_resp), g_vector

# -------------------------------------------------
# 5) Build Axis Lines (Color and Shape)
# -------------------------------------------------
...
# -------------------------------------------------
# 6) Visualization with Axis Overlays
# -------------------------------------------------
def visualize_four_subplots_with_axis(
    ...
    ax1.set_ylabel("PC2")
    ax1.grid(True)
    print("Unmod grid 2d[:,0]:")
    print(grid_unmod_2d[:,0])
    print("Unmod grid 2d[:,1]:")
    print(grid_unmod_2d[:,1])
    ax1.legend()
    ...
    print("grid_unmod_2d_innoise[:,0]:")
    print(grid_unmod_2d_innoise[:,0])
    print("grid_unmod_2d_innoise[:,1]:")
    print(grid_unmod_2d_innoise[:,1])
    ax2.legend()
    ...
    print("grid_mod_2d_innoise[:,0]:")
    print(grid_mod_2d_innoise[:,0])
    print("grid_mod_2d_innoise[:,1]:")
    print(grid_mod_2d_innoise[:,1])
...
if __name__=="__main__":
    ...
    # 3) Fit 2D PCA on unmod and extract pc1; flip if needed.
    ...
    # 4) Use the new PC1-based modulation (our "push color, pull shape" formula)
    responses_grid_mod_pc1, g_vector_pc1 = compute_modulated_responses_pc1_new(
        W_R_untuned, W_F, S, stimuli_grid, pc1, alpha=0.15
    )
    ...
    color_line_mod = compute_color_line(W_R_untuned, W_F, 0.0, color_line_vals, g_vector_pc1)
    shape_line_vals = np.linspace(0, 1, 10)
    shape_line_unmod = compute_shape_line(W_R_untuned, W_F, shape_line_vals, 0.0, g_vector=None)
    shape_line_mod = compute_shape_line(W_R_untuned, W_F, shape_line_vals, 0.0, g_vector_pc1)

    # 6) Visualize with overlays and print angles.
    visualize_four_subplots_with_axis(
        ...
        shape_line_unmod,
        shape_line_mod,
        title_main="2D PCA – PC1 Color Axis (Push Color, Pull Shape) with New Modulation"
    )
